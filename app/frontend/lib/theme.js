          ...
          input.addEventListener('change', function () {
            if (input.files && input.files[0]) {
              ...
              reader.onload = (e) => {
                ...
                let hasMultipleFiles = input.files.length > 1
                let multipleNameEnding = input.files.length === 2 ? 'other file' : 'others file'
                message.innerHTML = hasMultipleFiles ?
                  `${fileName} and ${input.files.length - 1} ${multipleNameEnding}`
                  : fileName
