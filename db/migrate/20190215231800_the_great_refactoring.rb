class TheGreatRefactoring < ActiveRecord::Migration[5.2]
  def change
    ...
    add_column :payments, :tax_rate, :integer, null: false
    add_column :payments, :tax_amount, :integer, null: false

    add_column :orders, :status, :string, null: false
    remove_column :orders, :tax_rate, :integer, null: false
    remove_column :orders, :date_created, :datetime, null: false, default: -> { 'clock_timestamp()' }
    ...
    add_column :order_items, :for_deposit, :boolean, null: false
